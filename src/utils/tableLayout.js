...
export function getTableCardLayout(cards, { compact = false } = {}) {
  ...
  return sorted.map((card, i) => {
    ...
    const radiusBase = compact
      ? (count <= 4 ? 52 : count <= 6 ? 62 : 72)
      : (count <= 4 ? 105 : count <= 6 ? 125 : 145);
    const radius = radiusBase + (compact ? h % 10 : h % 20);
...
export function getTableCardOffset(cardId, cards, options) {
  const layout = getTableCardLayout(cards, options);
